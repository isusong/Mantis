...
CFastConnectComponent::CFastConnectComponent(int width, int height)
:m_iwidth(width), m_iheight(height)
{
//	m_iwidth = IMAGE_WIDTH;
//	m_iheight = IMAGE_HEIGHT;
	m_ucmask = NULL;
	m_uclocalmask = NULL;
}
...
int CFastConnectComponent::FastConnectAlg( const unsigned char *inputMask, unsigned char *outputMask, 
										  const int &background,const int&valid)
{
	m_ucmask = inputMask;
	m_uclocalmask = outputMask;
	m_valid = valid & 0xff;
