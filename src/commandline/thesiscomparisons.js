...
var colsfolder = "centralplatecols/";
...
//0's are inserted for plates that don't exist, to make loading
//easier.
var cols = [672, 667, 668, 672, 667, 679, 668, 671, 675, 
			676, 683, 682, 663, 680, 674, 680, 682, 678, 
			670, 672, 684, 0, 678, 679, 671, 679, 667, 676,
			679, 679, 0, 683, 667, 667, 663, 674]; 
...
function vmark(tip, angle)
{
	this.tip = tip;
	this.angle = angle;
	this.profile = Profile(path + vmarkfolder
		+ "V" + tip + "-" + angle + ".csv");
}
function rmark(tip, angle, col)
{
	this.tip = tip;
	this.angle = angle;
	this.col = col;
	this.profile = Profile(path + colsfolder
		+ "P" + tip + "-" + angle
		+ "-Col" + col + ".csv");
}
	...
	for (var j = 0; j < rmarks.length; ++j)
	{
		...
		for (var k = 0; k < rmarks[j].length; ++k)
		{
			...
			for (var l = 0; l < vmarks[i].length; ++l)
			{
				...
				set.push(compare(stat, vmarks[i][l].profile, rmarks[j][k].profile));
				//Perform flipped comparison
				print("Flipped " + vmarks[i][l].angle);
				flippedset.push(compare(stat, vmarks[i][l].profile.flip(), rmarks[j][k].profile));
				setsum += set[l];
